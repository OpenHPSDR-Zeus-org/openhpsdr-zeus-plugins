...
//
// Ported from the in-core tests/Zeus.Server.Tests/WavFileSanitizerTests.cs.
// Now exercises the plugin's vendored AudioSanitize via WavWriter / WavFile —
// asserts NaN/Inf → 0 and hard-clamp to ±1 on the written/read float32 stream.
...
public sealed class WavFileSanitizerTests
{
    ...
    {
        string path = Path.Combine(Path.GetTempPath(), $"zeus-rec-writer-sanitize-{Guid.NewGuid():N}.wav");
        try
        {
    ...
    {
        string path = Path.Combine(Path.GetTempPath(), $"zeus-rec-read-sanitize-{Guid.NewGuid():N}.wav");
        try
        {
