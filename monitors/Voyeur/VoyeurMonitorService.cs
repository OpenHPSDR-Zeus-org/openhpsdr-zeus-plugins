...
///   PureSignal.
/// </summary>
public sealed class VoyeurMonitorService : IDisposable
{
    ...
    public VoyeurStatusDto Start(bool keepAudio = true)
    {
        lock (_ctrl)
        {
            ...
            string band = BandUtils.FreqToBand(freq) ?? "";
            var session = _store.CreateSession(freq, mode, band, keepAudio);
            ...
            // Set active LAST, after all state is built, so the first Feed()
            // from the host RX tap can't see a half-initialized ring.
            _active = true;
            ...
            _log.LogInformation(
                ...
                session.Id, freq, mode, band, keepAudio);
            return Status();
        }
    }
    ...
    // ---- PRODUCER (host RX-tap thread) — keep this trivially cheap and crash-proof ----
    // Fed by the plugin entry's IRxAudioTapPlugin.OnRxAudio. The receiver filter
    // (receiver 0 only) is applied by the caller via AudioBlockContext.Receiver;
    // the idle-guard (!_active) restores the "zero work when idle" property the
    // in-core version got from per-session event subscription. The host bridge
    // already wraps this call in try/catch, so a throw can't break RX audio.
    public void Feed(ReadOnlySpan<float> samples, int sampleRateHz)
    {
        if (!_active) return;
        var ring = _ring;
        ...
        try
        {
            // The ONLY work on the tap thread: a bounded copy into the ring.
            ring.Write(samples);
            _sampleRate = sampleRateHz;
        }
        catch
        {
            // Self-disable after repeated faults so a persistently failing ring
            // write becomes an inert no-op (the feature degrades; the radio runs
            // on). The bridge's per-tap try/catch already protects RX.
